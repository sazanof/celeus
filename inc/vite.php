...
function cssPreloadImports(string $entry): string
{
	if (isDev($entry)) {
		return '';
	}
	...
	$res = '';
	foreach (importsUrls($entry, true) as $url) {
...
{
	$content = file_get_contents('../public/dist/.vite/manifest.json');
...
function importsUrls(string $entry, $style = false): array
{
	$urls = [];
	$manifest = getManifest();
			...
			if (isset($manifest[$import])) {
				if ($style) {
					if (isset($manifest[$import]['css']) && is_array($manifest[$import]['css'])) {
						...
						$urls = array_merge(cssUrls($import), $urls);
						//}
					}

				} else {
					$urls[] = assetUrl($import);
				}
